...
randomSampler::randomSampler(Camera *c_ptr, ViewPlane *v_ptr){
    
    camera_ptr = c_ptr;
    viewplane_ptr = v_ptr;
}

randomSampler::randomSampler(const randomSampler &camera)
    : Sampler(camera.camera_ptr, camera.viewplane_ptr)
{}



randomSampler& randomSampler::operator=(const randomSampler& rhs){
    if(this == &rhs){
        return (*this);
    }
