...
bool BBox::intersects(Geometry* g) {
    return this->intersects(g->getBBox());
}

bool BBox::intersects(const BBox& b) {
    const bool xSeparated = this->pmin.x > b.pmax.x || b.pmin.x > this->pmax.x;
    const bool ySeparated = this->pmin.y > b.pmax.y || b.pmin.y > this->pmax.y;
    const bool zSeparated = this->pmin.z > b.pmax.z || b.pmin.z > this->pmax.z;

    return !xSeparated && !ySeparated && !zSeparated;
}
